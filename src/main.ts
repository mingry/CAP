...
type EnemyKind = 'scout' | 'zigzag' | 'boss';
...
const CONFIG = {
  ...
  spawnEvery: 1.05,
  playerArmor: 3,
  worldX: [-10, 12] as const,
  worldY: [-5.2, 5.2] as const,
};

const canvas = document.querySelector<HTMLCanvasElement>('#game-canvas')!;
const scoreElement = document.querySelector<HTMLElement>('#score')!;
const armorElement = document.querySelector<HTMLElement>('#armor')!;
...
function createPlayer(): THREE.Group {
  ...
  const body = new THREE.Mesh(
    new THREE.ConeGeometry(0.62, 2.7, 6),
    new THREE.MeshStandardMaterial({ color: '#f4b860', metalness: 0.55, roughness: 0.3 }),
  );
  body.rotation.z = -Math.PI / 2;
  group.add(body);
  const wing = new THREE.Mesh(
    new THREE.BoxGeometry(1.25, 0.12, 1.8),
    new THREE.MeshStandardMaterial({ color: '#e9eff0', metalness: 0.5, roughness: 0.36 }),
  );
  wing.position.x = -0.2;
  group.add(wing);
  const engine = new THREE.Mesh(
    new THREE.CylinderGeometry(0.2, 0.32, 0.62, 8),
    new THREE.MeshBasicMaterial({ color: '#ff765c' }),
  );
...
function createEnemy(kind: EnemyKind): Enemy {
  const group = new THREE.Group();
  ...
  const ring = new THREE.Mesh(
    new THREE.TorusGeometry(kind === 'boss' ? 1.25 : 0.52, 0.08, 6, 12),
    new THREE.MeshBasicMaterial({ color: kind === 'boss' ? '#f4b860' : '#d9edf0' }),
  );
...
function createProjectile(friendly: boolean): void {
  const mesh = new THREE.Mesh(
    new THREE.SphereGeometry(friendly ? 0.13 : 0.17, 8, 8),
    new THREE.MeshBasicMaterial({ color: friendly ? '#f4b860' : '#ff5e70' }),
  );
  const start = friendly ? player.position.clone() : new THREE.Vector3(0, 0, 0);
  mesh.position.copy(start);
  if (!friendly) mesh.position.set(boss?.group.position.x ?? 8, boss?.group.position.y ?? 0, 0.2);
  world.add(mesh);
...
function endGame(nextState: 'gameOver' | 'victory'): void {
  state = nextState;
...
function updateStars(delta: number): void {
  for (const star of stars) {
    ...
    if (star.position.x < -13) star.position.x = 13;
  }
}

function updateSpawning(delta: number): void {
...
function updateEnemies(delta: number): void {
  for (const enemy of [...enemies]) {
    ...
    if (isBoss) {
      ...
      if (enemy.kind === 'zigzag') enemy.group.position.y += Math.sin(elapsed * 3 + enemy.phase) * delta * 2.7;
      enemy.group.rotation.y += delta * 2;
      if (enemy.group.position.x < -12) removeEnemy(enemy);
    }
    if (enemy.group.position.distanceTo(player.position) < enemy.radius + 0.58) {
      damagePlayer();
      if (!isBoss) removeEnemy(enemy);
    }
  }
}
    ...
    for (const enemy of [...enemies]) {
      if (projectile.mesh.position.distanceTo(enemy.group.position) < projectile.radius + enemy.radius) {
        ...
        if (enemy.health <= 0) {
          ...
          if (enemy.kind === 'boss') endGame('victory');
          removeEnemy(enemy);
        }
        break;
      }
...
function updateHud(): void {
  ...
  if (elapsed < CONFIG.introDuration && state === 'playing') phaseElement.textContent = 'THREAT WINDOW';
}
